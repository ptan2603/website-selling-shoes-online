...
// định nghĩa mảng tên là  chứa các danh sách và thông tin
const brands = [ 
  {
    display_name: "PUMA",
    value: "1",
...
// biểu thị một số lượng sản phẩm hiển thị
const count = 12;
// một mảng chứa danh sách giá trị đại diện
...
const Product = (props) => {
  ...
  // mảng rows chứa các phần tử total .map để biến đổi mỗi phần tử trong mảng và tạo ra một mảng mới chứa kết quả biến đổ.
...
//được sử dụng để lấy danh sách sản phẩm từ API dựa trên các danh mục, thương hiệu, giá và page.
// mỗi khi trang thay đổi gọi lại để lấy dữ liệu sản phẩm mới
  useEffect(() => {
    if (category.length === 0 && brand.length === 0 && price.length === 0) {
    getAllProducts(page, count, true).then((response) => {
        setProducts(response.data.content); // dữ liệu sản phẩm
    ...
    // để lấy danh sách sản phẩm đã lọc và cập nhật trạng thái
...
// hàm này dùng để lọc các giá trị và cập nhật lại giá trị đã chọn
  const chooseBrandHandler = (value) => {
    ...
    onChangePage(1); // cập nhật lại về trang đầu tiên và hiển thị dữ liệu đã chọn
  };

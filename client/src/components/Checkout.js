...
const Checkout = (props) => {
  ...
  const onLoad = () => {
    ...
    if (props.user) {
      setCart(
        props.cartItem.filter((item) => props.buy.includes(item.id.toString()))
      );
        const result = props.cartItem
          .filter((item) => props.buy.includes(item.id.toString()))
          .reduce(
            (total, item) =>
              total + item.price, // Tính tổng giá trị cho từng mặt hàng dựa trên giá
            0
          );
        setAmount(result);
      ...
      const result = props.cartItem
        ...
        .reduce(
          (total, item) =>
              total + item.price * item.quantity, // Tính tổng giá trị cho từng mặt hàng dựa trên giá
            0
        );
      setAmount(result);
    }
    ...
    <div className="pb-3 container-fluid">
      ...
      <div className="row">
        <div className="col-md-5 col-lg-4 order-md-last">
          ...
          <NavLink to="/giohang" className={cart.length === 0 ? "mb-2 mr-5 disabled" : "mb-2 mr-5"} exact>
            Quay về giỏ hàng
          </NavLink>
        </div>
